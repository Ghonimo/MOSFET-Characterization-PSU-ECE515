...
% Select data in the range 0.6V <= V_GS <= 1V
indices_VT = find((V_GS_target >= 0.6) & (V_GS_target <= 1));
...
text(min(V_GS_target_VT), min(log10(I_D_target_VT)), str, 'FontSize', 12);
hold off;

xlabel('V_G_S (V)');
ylabel('log_{10}(I_D) ');
title('log_{10}(I_D) vs V_G_S for 0.6V <= V_G_S <= 1V');
